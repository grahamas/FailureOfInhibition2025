...
function solve_augmented_trace_zero(config, parameters, axis, seed)
    ...
    in_bounds = result.success && config.bounds[axis][1] <= result.candidate[3] <=
        config.bounds[axis][2]
    full_rank = result.success && result.minimum_singular_value > config.augmented.rank_atol
    return merge(result, (; success=result.success && in_bounds && full_rank,
        in_bounds, full_rank))
end
...
function continue_axis_path(config, seed, axis; smoke=false)
    ...
    start = try
        _equilibrium_step(config, seed.parameters, seed.central_state)
    catch error
        error isa InterruptException && rethrow()
    ...
    for direction in (-1, 1)
        ...
        while accepted_points < max_steps
            target = parameter + direction * step
            if target < bounds[1] || target > bounds[2]
                termination = "parameter_bound"
                break
            end
            trial_parameters = replace_parameter(seed.parameters, axis, target)
            trial = try
                _equilibrium_step(config, trial_parameters, state)
            catch error
                error isa InterruptException && rethrow()
            ...
            if isfinite(previous_trace) && previous_trace * trial.trace <= 0
                ...
                solved = solve_augmented_trace_zero(config, seed.parameters, axis,
                    augmented_seed)
                ...
                push!(locations, (; axis, direction, success=solved.success,
                    ...
                    in_bounds=solved.in_bounds, full_rank=solved.full_rank,
    ...
    for direction in (-1, 1)
        ...
        while accepted < max_steps
            ...
            for retry in 0:max_retries
                ...
                corrected = _curve_corrector(residual_function, predictor,
                ...
                boundary_encountered |= corrected.success && !in_bounds
            ...
            if !succeeded
                termination = boundary_encountered && step < minimum_step ?
...
function assess_directional_hopf(config, parameters, state, direction, reference_track)
    ...
    if length(reference_states) != 3 ||
        ...
        return (; resolved=false, beta=NaN, values=Float64[], rows=NamedTuple[],
            ...
            diagnostics, reasons=[:invalid_reference_central_track],
    ...
    if !predictor.resolved
        return (; resolved=false, beta=NaN, values=Float64[], rows=NamedTuple[],
            errors=NamedTuple[], spread=Inf, tolerance=NaN, rho_squared_slope=NaN,
            c1_real=diagnostics.c1 === nothing ? NaN : real(diagnostics.c1),
            diagnostics, reasons=unique(vcat([:directional_predictor_unresolved],
    ...
    diagnostics.resolved || push!(reasons, :lyapunov_unresolved)
    c1_real = diagnostics.c1 === nothing ? NaN : real(diagnostics.c1)
    isfinite(c1_real) && abs(c1_real) > eps(Float64) || push!(reasons, :degenerate_c1)
    ...
    return merge(estimate, (; resolved=isempty(reasons), rho_squared_slope,
        c1_real, diagnostics, reasons, direction=normalized, samples, predictor,
...
function run_experiment(config_path::AbstractString,
    ...
    if replay_mode
        replay_root = _canonical_destination(replay.root)
        relative = relpath(output, replay_root)
        outside = relative == ".." || startswith(relative, "../") || startswith(relative, "..\\")
        (relative == "." || !outside) && throw(ArgumentError(
            "replay output must be outside the source artifact"))
    end
    ...
    for seed in manifest
        ...
        searches = try
            ...
            push!(seed_rows, (; hypothesis_id=seed.hypothesis_id, qualified=false,
                reasons="execution_exception", central_E=NaN, central_I=NaN))
            continue
        end
        ...
        push!(seed_rows, (; hypothesis_id=seed.hypothesis_id,
            qualified=topology.qualified, reasons=join(string.(topology.reasons), ";"),
        ...
        for axis in config.path_axes
            ...
            push!(axis_branch_rows, (; branch_id, hypothesis_id=seed.hypothesis_id,
                ...
                locations=length(result.locations), termination=result.termination))
    ...
    for (index, location) in enumerate(deduplicated.representatives)
        identifier = hypothesis_hash(location.parameters, config.fixed,
            protocol_source_hash, protocol_config_hash,
            location.parent_hypothesis, "trace_zero:$(location.method)";
            state=location.state)
        _write_hypothesis(output, identifier, Dict("hypothesis_id" => identifier,
            "parent_hypothesis" => location.parent_hypothesis,
            "method" => location.method, "parameters" => location.parameters,
            "state" => location.state, "source_id" => location.source_id))
    ...
    Evidence.write_rows(joinpath(output, "seed_topologies.csv"), seed_rows,
        (:hypothesis_id, :qualified, :reasons, :central_E, :central_I))
    ...
    unexhausted_axis = count(row -> !exhausted_termination(row.termination),
        axis_branch_rows)
    ...
    numerical_outcome = isempty(deduplicated.representatives) && unexhausted_branches == 0 ?
        "completed_no_trace_zero_solution" : accepted_count > 0 ?
        "completed_with_validated_ratio44_orbit" :
        qualified_unattempted > 0 || unexhausted_branches > 0 ?
        "completed_unresolved_targeted_search" : "completed_no_validated_ratio44_orbit"
